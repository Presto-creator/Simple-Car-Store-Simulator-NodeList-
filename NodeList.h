...
public:
	...
	public:
		...
		/*-----------------------------------------------------------------*/
		Iterator operator++(int) // p++ post increment operator overloading
		{
...
public:
	...
	Elem& at(int id) const;							// easy notation for accessing an element object
												// housekeeping functions omitted...
	/*===Rule of three ==================*/
	~NodeList();   /* destructor*/
	NodeList(const NodeList<Elem>& obj); /*copy constructor*/
	//overloading assignment operator
	NodeList<Elem>& operator=(const NodeList<Elem>& rhs); /* assignment operator*/
	/*===================================*/

	 NodeList<Elem>& operator[](int id);
	 
	 //const Elem& operator[](int id);
...
template<class Elem>
void NodeList<Elem>::clear() // clear the whole nodelist
{
...
//===Rule of three ==================
template <class Elem>
NodeList<Elem>::~NodeList()   // destructor
{
	while (!empty())
		eraseFront();
	delete header;
...
template <class Elem>
NodeList<Elem>::NodeList(const NodeList<Elem>& obj) {
	header = new Node;       // create sentinels
	trailer = new Node;
	header->next = trailer;      // have them point to each other
	trailer->prev = header;
	...
	n = 0;
	if (obj.header != obj.trailer) {

		for (Node* curr = obj.header->next; curr != obj.trailer; curr = curr->next)
		{
			insertBack(curr->elem);
...
//overloading assignment operator
template <class Elem>
NodeList<Elem>& NodeList<Elem>::operator=(const NodeList<Elem>& rhs) // assignment operator
{
	if (this != &rhs) {
		n = 0;
		while (!empty()) eraseFront(); //clear 
		for (Node* curr = rhs.header->next; curr != rhs.trailer; curr = curr->next)
		{
			insertBack(curr->elem);
...
template<class Elem>
Elem& NodeList<Elem>::at(int id) const
{
	Iterator itr = begin();
	for (int i = 0; i < id; ++i) {
		++itr;
...
/*
template<class Elem>
NodeList<Elem>& NodeList<Elem>::operator[](int id)
{
	Iterator itr = begin();
	for (int i = 0; i < id; ++i) {
		++itr;
...
template<class Elem>
const Elem& NodeList<Elem>::operator[](int id)
{
	Iterator itr = begin();
	for (int i = 0; i < id; ++i) {
		++itr;
